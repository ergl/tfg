...
\todo{This is actually each partition. We should make clear that a server might be responsible for more than one partition, or at least use consistent naming through the chapter to distinguish servers from partitions.} Each server $s_i$ maintains five main data structures: $\lastprep$, $\VersionLog$, $\CommitLog$, $\LocalTime$ and $\CommitQueue$. Their role is summarised in Figure~\ref{fig:prot-ds-table}. We follow with a more detailed explanation of each of them.
...
% Recall from~\ref{sect:protocol_structures} the usage of version vectors to represent causal dependencies, such that two transactions $\tx_1$ and $\tx_2$ are causally dependent if $\tx_1.\Vcomm \sqsubseteq \tx_2.\VCdep$\todonote{Revisit, true, or not?}, i.e. $\tx_2$ is causally dependent on $\tx_1$ if $\tx_2$ observes the updates of $\tx_1$. However, when a transaction $\tx$ fixes a snapshot at partition $i$, the possible causal dependencies of $\tx$ are computed according to its snapshot vector, $\tx.\VCaggr$, which represents the join of the commit vectors of transactions that committed at the partitions that $\tx$ read before. In practice, this means that

The strict per-partition commit order requirement of SI, combined with fastPSI's usage of version vectors with one entry per partition to represent causal dependencies, make any two transactions that commit at the same partition causally dependent on each other, even if they are not aware of each other's updates\footnote{As introduced in the \emph{causal dependency} definition from~\ref{sect:histories}.}. This limitation---caused by the coarse-grained nature of version vectors---combined with the different commit orderings of PSI, can cause pathological orderings such that any snapshot that observes them will not be causally consistent. \todo{Verify following, only true under particular circumstances} In particular, the strict per-partition ordering inherited from SI precludes transactions from observing long fork anomalies across partitions.
