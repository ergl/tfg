...
\caption{List of variables used in the protocol, where
  ...
  with the commit order of the associated transactions. We
  select the components of various tuples using the names given in the figure.}
\label{fig:prot-ds-table}
\end{figure}

\todo{This is actually each partition. We should make clear that a server might be responsible for more than one partition, or at least use consistent naming through the chapter to distinguish servers from partitions.} Each server $s_i$ maintains five main data structures, summarised in Figure~\ref{fig:prot-ds-table}. We now offer a more detailed explanation of each of them.
