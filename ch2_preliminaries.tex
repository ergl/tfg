...
A \emph{Write Skew} occurs whenever two transactions concurrently update a disjoint set of objects that were originally related by an external constraint. Suppose that objects $x$ and $y$ are related by a constraint such that $x + y < N$, with two concurrent transactions $\tx_i$ and $\tx_j$ originally observing a consistent state. $\tx_i$ proceeds to update $x$, while satisfying the original constraint. $\tx_j$ does the same by updating $y$. After both $\tx_i$ and $\tx_j$ commit, it might be the case that the original constraint no longer holds. Figure~\ref{fig:write_skew_history} shows an example of such a history.
\end{definition}
