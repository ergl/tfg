...
\subsection{Histories and Executions}

We call a \textbf{\em history} $\Hist$ to the finite set of all transactions with disjoint identifiers issued against a database. An \textbf{\em execution} is a tuple $\Exe = (\Hist, \hb)$, where $\Hist$ is a history, and \textbf{\em happens-before} $\hb$ is a strict partial order over $\Hist$.

Thus, a history records all the client-database interactions that occur during a particular program. For simplicity, we consider only finite histories. An execution enriches a history with a happens-before order, describing how the database processes transactions. Intuitively $\tx_i \hb \tx_j$ means that $\tx_j$ is aware of the updates performed by $\tx_i$, and thus the outcome of the operations in $\tx_j$ may depend on the effects of $\tx_i$. In this case, we also say that $T_i$ is a \textbf{\em causal dependency} of $\tx_j$.
...
\begin{tabularx}{\linewidth}{ >{\centering}p{8cm} | *{5}{>{\centering}X}}
    ...
    Non-Repeatable Reads & x & x & x & x & - \tabularnewline
    ...
    Real-Time Violation & \checkmark & x & \checkmark & \checkmark & \checkmark \tabularnewline
\end{tabularx}
\end{center}
