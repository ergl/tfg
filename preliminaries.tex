...
\begin{figure}[h]
  ...
  \vspace{-1cm}
  \caption{Example of history represented as a graph. Arrows represent the causal dependencies between operations. Adapted from \em{Saeida Ardekani et al.~\citep{ardekani-nsmi}}.}
  \label{fig:history}
\end{figure}
...
\begin{figure}[h]
  ...
  \vspace{-0.5cm}
  \caption{Example of a history showing the long fork anomaly. Transaction $\tx_3$ observes $\tx_1 \rightarrow \tx_2$ while $\tx_4$ observes $\tx_2 \rightarrow \tx_1$. Adapted from \em{Saeida Ardekani et al.~\citep{ardekani-nsmi}}.}
