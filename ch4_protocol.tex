...
\todo{Move table closer to the text}

\todo{Add an ``overview'' section with a figure that explains the protocol at a high level, step by step. We can do it as an example, denoting the messages between client and server. We can also say how the state (assuming an initial one) evolves with each step.
...
\todo{
  \begin{itemize}
    \item Split the pseudocode and inline it with the text.
