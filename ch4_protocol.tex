...
Our protocol follows a client-server model with optimistic concurrency control: clients execute transactions speculatively by issuing remote read operations to servers and buffering writes locally. At the end of the execution of a transaction, clients perform a variant of the classical \emph{two-phase commit protocol (2PC)} among all the participating servers to decide whether to commit or abort the transaction. Figure~\ref{fig:execution} shows an overview of an example execution of the protocol.
...
\end{algorithm}
\caption{Partition state update after at server $s_i$ after a transaction commits}
\end{figure}
