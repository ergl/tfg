...
\todo{Split the pseudocode and inline it with the text}

\todo{Think if \emph{causal dependency} is already defined in a previous section}

\todo{Double check consistent usage of $i$ and $j$}
...
\caption{Illustrations of the snapshot computation. Vertical lines depict the
  commit order at the corresponding partitions (top to bottom) and horizontal
  lines the cut-offs of various snapshots. Arrows between partitions depict
  causal dependencies. \todo{Move to a logical page once section is done}}
\label{fig:snapshot}
\end{figure}

\todo{Example of simple read abort?}
...
It may be the case that it's impossible to satisfy both of the above requirements when selecting a snapshot; e.g., this is the case in the situation illustrated in figure~\ref{fig:snapshot}b. Assuming that the transaction $\tx$ has fixed a valid snapshot at $j$ and $k$, it is impossible to build a consistent snapshot at partition $i$: because $\tx$ included $\tx_3$ at partition $k$, it is forced to read $\tx_4$ writes at $i$. At the same time, because it excluded $\tx_1$ from the snapshot at $j$, $\tx$ can't read the writes by $\tx_2$ at $k$. In this case, the server $s_i$ will build a snapshot that excludes both $\tx_2$ and $\tx_4$, therefore violating our second requirement. \todo{Verify this, I \emph{think} that the snapshot retuned would be before $\tx_2$, but can't be sure}. This this case, the server sends to the client a $\READRETURN$ message with a special value $\abort$ (line~\ref{alg:read_req_abort}), which will cause the client to abort the transaction (line~\ref{alg:read_abort}).
