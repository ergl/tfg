...
\todo{Mention partitions. How many? Fixed? Mention only on the experiments?}
...
\todo{Discuss that naiveSER/fastPSI's scalability bottleneck is at the partition level, the smallest unit of concurrency for both reads (fixing a snapshot) and updates (2PC). For naiveRC, the smallest unit of concurrency for reads is the key, and for updates, the partition.}
